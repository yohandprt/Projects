...
## File description:
## script shell that test the 42sh project
##
...
compare ()
{
    if [ "$1" = "$2" ] && [ "$3" = "$4" ]; then
        ...
        if [ "$3" -eq 139 ] || [ "$3" -eq 134 ]; then
            printf "%d -\t\033[1;31m[CRASH]\033[0m  \033[4;31m(%s)\n\033[0m" $TOTAL_TESTS "$5"
            CRASH=$((CRASH+1))
        else
...
then
    ...
    printf "\033[1;34mFailed: %d/%d\033[0m\n\n" $((TOTAL_TESTS-SUCCESS_TESTS)) $TOTAL_TESTSen
    printf "\033[1;31m[CRASH]\033[0m %d/%d\n" $CRASH $TOTAL_TESTS
